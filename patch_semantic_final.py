...
code = code.replace("namespace semantic {", "namespace semantic {" + helper)
...
# Fix unhandled nodes
unhandled_old = """    } else if (root->node_type == parser::NodeType::EXPRESSION_STATEMENT) {
...
unhandled_new = """    } else if (root->node_type == parser::NodeType::EXPRESSION_STATEMENT) {
        evaluateExpression(tree, static_cast<parser::ExpressionStatement*>(root)->expression.get());
    } else if (root->node_type == parser::NodeType::RETURN_STATEMENT) {
        auto return_stmt = static_cast<parser::ReturnStatement*>(root);
        if (return_stmt->value) {
