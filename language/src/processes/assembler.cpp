...
void Assembler::apply_linker_patches() {
  for (auto &patch : linker_patches) {
    ...
    if (function_ips.find(target) != function_ips.end()) {
      ...
      if (target->node_type == NodeType::METHOD_DECL)
        name = static_cast<MethodDeclaration *>(target)->mangled_name;
      else if (target->node_type == NodeType::CONSTRUCTOR_DECL)
        name = static_cast<ConstructorDeclaration *>(target)->mangled_name;
      throw std::runtime_error(
          "Linker Error: Target function not found for patching: " + name);
    }
  }
}
...
void Assembler::compile_function(Node *function_node) {
  ...
  if (function_node->node_type == NodeType::METHOD_DECL) {
    ...
    if (!m->template_parameters.empty())
      return; // SHIELD: Skip compiling method blueprints
...
void Assembler::visit(ClassDeclaration &node) { compile_class(&node); }
...
void Assembler::visit(IfStatement &node) {
  auto *if_stmt = &node;
...
void Assembler::visit(WhileStatement &node) {
  auto *while_stmt = &node;
...
void Assembler::visit(ForStatement &node) {
  auto *for_stmt = &node;
...
void Assembler::visit(DoWhileStatement &node) {
  auto *do_while = &node;
  uint32_t loop_start_ip = bytecode().size();
...
void Assembler::visit(SwitchStatement &node) {
  ...
  for (const auto &child : switch_stmt->children) {
    ...
    if (case_stmt->is_default) {
      ...
      bytecode()[patch_ip + 2] = (current_ip >> 8) & 0xFF; // Fixed typo here
...
void Assembler::visit(LiteralNode &node) {
  auto *lit = &node;
...
void Assembler::visit(IdentifierNode &node) {
  auto *ident = &node;
...
void Assembler::visit(AssignmentExpression &node) {
  auto *assign = &node;
...
void Assembler::visit(MethodCallExpression &node) {
  ...
  } else if (call->resolved_declaration->node_type ==
             NodeType::CONSTRUCTOR_DECL) {
  ...
  // Determine if this target actually requires an implicit 'this' instance
  bool is_instance_method =
      !is_static_method && call->resolved_declaration->parent != nullptr &&
      call->resolved_declaration->parent->node_type == NodeType::CLASS_DECL;
...
void Assembler::visit(MemberAccessExpression &node) {
  auto *mem_acc = &node;
  ...
  if (!field) {
    throw std::runtime_error(
        "MEMBER_ACCESS read resolved_declaration is null at line " +
        std::to_string(mem_acc->line));
  }
...
void Assembler::visit(ArrayAccessExpression &node) {
  auto *arr_acc = &node;
  compile_expression(arr_acc->array.get());
  compile_expression(arr_acc->index.get());
...
void Assembler::visit(ArrayCreationExpression &node) {
  auto *arr_crea = &node;
  compile_expression(arr_crea->size.get());
  emit_byte(static_cast<uint8_t>(OpCode::ALLOC_DYNAMIC));
}

void Assembler::visit(ArrayLiteralExpression &node) {
  auto *arr_lit = &node;
  emit_byte(static_cast<uint8_t>(OpCode::PUSH_CONST_I32));
...
void Assembler::visit(CastExpression &node) {
  auto *cast_expr = &node;
  ...
  if (t == "int8")
    ...
    emit_byte(static_cast<uint8_t>(OpCode::CONV_I64));
  else if (t == "uint8")
    ...
    emit_byte(static_cast<uint8_t>(OpCode::CONV_U64));
  else if (t == "float32")
    emit_byte(static_cast<uint8_t>(OpCode::CONV_F32));
  else if (t == "float64")
    emit_byte(static_cast<uint8_t>(OpCode::CONV_F64));
...
void Assembler::visit(InstanceofExpression &node) {
  auto *inst_expr = &node;
...
void Assembler::visit(TernaryExpression &node) {
  auto *tern = &node;
