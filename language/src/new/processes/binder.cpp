...
void Binder::throw_error(Node* node, const std::string& msg) {
    ...
    if (node && node->source) {
        if (std::holds_alternative<std::filesystem::path>(*node->source)) {
            error_message += std::get<std::filesystem::path>(*node->source).string() + ":";
        } else {
            error_message += std::get<std::string>(*node->source) + ":";
        }
        error_message += std::to_string(node->line) + ":" + std::to_string(node->column) + " - ";
    }
    error_message += msg;
...
void Binder::register_members(Node* node, const std::string& prefix) {
    ...
    if (node->node_type == NodeType::PACKAGE_STMT) {
        ...
        if (current_class) {
            log_info("Injected 'this' into constructor of {}", current_class->class_name);
        }
...
TypeInfo Binder::resolve_type(const TypeInfo& raw_type, Node* error_node) {
    if (raw_type.name.empty()) return raw_type;
...
void Binder::bind_node(Node* node) {
    ...
    if (node->node_type == NodeType::BLOCK) {
        ...
        if (var->initializer) {
            ...
            if (initializer_type != var->type_info) {
                throw_error(node, "Type mismatch in variable declaration: expected '"
                    + var->type_info.name + "', got '" + initializer_type.name + "'");
            }
        }
        ...
        if (condition_type.name != "bool") throw_error(node, "Condition must be bool");
        ...
        loop_depth--;
        TypeInfo condition_type = evaluate_expression(do_while_stmt->condition.get());
        if (condition_type.name != "bool") throw_error(node, "Condition must be bool");
        ...
        if (for_stmt->initialization) bind_node(for_stmt->initialization.get());
        if (for_stmt->condition) {
            TypeInfo condition_type = evaluate_expression(for_stmt->condition.get());
            if (condition_type.name != "bool") throw_error(node, "Condition must be bool");
        }
        if (for_stmt->iteration) evaluate_expression(for_stmt->iteration.get());
        loop_depth++;
        ...
        if (case_stmt->case_value) evaluate_expression(case_stmt->case_value.get());
        ...
        if (return_stmt->value) {
            ...
            if (current_method && return_type != current_method->return_type) {
                throw_error(node, "Return type mismatch: expected '"
                    + current_method->return_type.name + "', got '" + return_type.name + "'");
            }
        ...
        if (loop_depth == 0) throw_error(node, "Continue must be inside a loop");
...
TypeInfo Binder::evaluate_expression(Node* expr) {
    ...
    if (expr->node_type == NodeType::LITERAL) {
        ...
        if      (std::holds_alternative<int64_t>(lit->value))     expr->expression_type = {"int32",   0};
        ...
        if (!declaration && current_class) {
            declaration = global_scope.resolve(current_class->mangled_name + "." + id->name);
        }
        ...
        if (!declaration) throw_error(expr, "Undefined identifier: " + id->name);
        expr->resolved_declaration = declaration;

        if (declaration->node_type == NodeType::VAR_DECL) {
            expr->expression_type = static_cast<VariableDeclaration*>(declaration)->type_info;
        } else if (declaration->node_type == NodeType::FIELD_DECL) {
            expr->expression_type = static_cast<FieldDeclaration*>(declaration)->type_info;
        ...
        if (left_type != right_type) {
            throw_error(expr, "Binary operands type mismatch: '"
                + left_type.name + "' vs '" + right_type.name + "'");
        }
        ...
        if (condition_type.name != "bool") throw_error(expr, "Ternary condition must be bool");
        ...
        if (!type_decl) throw_error(expr, "Cannot access members on unknown type: " + object_type.name);

        if (type_decl->node_type == NodeType::CLASS_DECL) {
            auto* class_decl = static_cast<ClassDeclaration*>(type_decl);
            Node* member_decl = global_scope.resolve(
                class_decl->mangled_name + "." + member_access->member_name);
            if (!member_decl) {
            ...
            // Nested class or enum access — return the type itself so further chained access works.
        ...
        if (method_call->callee->node_type == NodeType::MEMBER_ACCESS) {
            auto* member_access = static_cast<MemberAccessExpression*>(method_call->callee.get());
            TypeInfo receiver_type = evaluate_expression(member_access->object.get());
            std::string base_name = receiver_type.name + "." + member_access->member_name;
            std::string mangled_name = base_name;
            for (const auto& arg_type : argument_types) mangled_name += "_" + arg_type.to_string();

            // Try mangled name first, then fall back to plain name for native methods.
            Node* method_decl = global_scope.resolve(mangled_name);
            if (!method_decl) method_decl = global_scope.resolve(base_name);
            if (!method_decl) throw_error(expr, "No matching method: " + mangled_name);
            ...
            if (!current_class) throw_error(expr, "Local method calls must be inside a class");
        ...
        new_instance->type_info = resolve_type(new_instance->type_info, new_instance);
        ...
        std::string mangled_ctor = mangle_constructor(new_instance->type_info.name, argument_types);
        Node* ctor = global_scope.resolve(mangled_ctor);

        // Fallback: find any constructor with the same arity (allows numeric coercion).
        ...
        new_instance->resolved_declaration = global_scope.resolve(new_instance->type_info.name);
        ...
        array_creation->type_info = resolve_type(array_creation->type_info, array_creation);
        TypeInfo size_type = evaluate_expression(array_creation->size.get());
        if (size_type.name != "int32") throw_error(expr, "Array size must be int32");
        ...
        if (array_type.array_depth == 0) throw_error(expr, "Cannot index a non-array value");
        TypeInfo index_type = evaluate_expression(array_access->index.get());
        if (index_type.name != "int32") throw_error(expr, "Array index must be int32");
