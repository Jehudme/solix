...
## 13. Developer Tooling & CLI Ecosystem

- [ ] **Project Manifest & Build System (`solix.toml` / `solix.json`)**
  - **Issue**: Compiling applications currently requires specifying individual `.slx` files manually from the command line. Large projects need a declarative manifest.
  - **Fix**:
    - Introduce a declarative project manifest (`solix.toml` or `solix.json`) specifying:
      - Project metadata: name, version, author, license.
      - Entry point file and entry function.
      - Source directories, output binary path, and compilation flags.
      - Dependencies and standard library inclusion options.
    - Implement CLI project management subcommands:
      - `solix new <project_name>`: Scaffolds a new project with standard directory structure (`src/`, `lib/`, `solix.toml`).
      - `solix build`: Discovers all project sources and compiles to `.slxb` based on manifest configuration.
      - `solix run [args...]`: Builds and immediately executes the target project.
      - `solix test`: Runs all project tests in `tests/`.
      - `solix clean`: Removes build artifacts and cached bytecode.

- [ ] **Language Server Protocol (LSP) & Editor Extension (`solix lsp`)**
  - **Goal**: Deliver a first-class IDE developer experience for VS Code, Neovim, and other LSP-compatible editors.
  - **Features**:
    - Implement `solix lsp` server subcommand implementing the JSON-RPC Language Server Protocol.
    - **Diagnostics**: Push real-time lexer, parser, and binder errors/warnings as the user types.
    - **Hover Information**: Display type signatures, docstrings, and parameter lists on hover.
    - **Go to Definition**: Jump from method/field/type references to their declarations across packages.
    - **Document Symbols & Outline**: Provide class, method, and variable outlines for the file.
    - **VS Code Extension**: Publish a syntax highlighting and LSP client extension for `.slx` files.

- [ ] **Interactive REPL (`solix repl`)**
  - **Goal**: Allow developers to interactively experiment with language syntax, test functions, and inspect values.
  - **Features**:
    - Launch interactive shell with `solix repl` (or running `solix` without arguments).
    - Immediate evaluation of expressions and statements.
    - Persistent session scope retaining declared variables, functions, and classes across prompts.
    - Special commands: `:help`, `:load <file.slx>`, `:reset`, `:types`, `:exit`.

- [ ] **Code Formatter (`solix fmt`)**
  - **Goal**: Maintain clean, consistent, and idiomatic code formatting across standard library files and user projects.
  - **Features**:
    - Implement `solix fmt [files...]` subcommand.
    - Enforce standard Solix formatting rules: indentation (4 spaces), bracket placement, operator spacing, line length wrapping, and import/alias grouping.
    - Support `--check` flag for CI pipelines to fail if files are unformatted.

---

## 14. VM & Compiler Performance Optimizations

- [ ] **Bytecode Peephole Optimizer in Assembler**
  - **Goal**: Optimize generated bytecode sequences prior to serialization to reduce binary size and execution time.
  - **Optimizations**:
    - **Constant Folding**: Evaluate arithmetic on constant literals at compile-time (e.g. `2 + 3` $\to$ `PUSH_CONST_I32 5`).
    - **Redundant Load/Store Elimination**: Eliminate adjacent `SET_LOCAL x; GET_LOCAL x;` patterns using `DUP`.
    - **Instruction Merging & Specialization**:
      - Replace `PUSH_CONST_I32 1` + `ADD_I64` with specialized `INC_I64`.
      - Replace `PUSH_CONST_I32 1` + `SUB_I64` with specialized `DEC_I64`.
      - Replace `PUSH_CONST_I32 0` + `EQ_I64` with `IS_ZERO_I64`.
    - **Dead Code Elimination**: Strip unreachable bytecode instructions following unconditional `RETURN`, `THROW`, or `JMP`.

- [ ] **Memory & ARC Profiler / Leak Detector**
  - **Goal**: Provide deep runtime visibility into heap usage, allocation rates, and memory integrity.
  - **Features**:
    - Add VM diagnostic flags: `--trace-gc` and `--profile-mem`.
    - **Heap Tracking**: Track current heap allocations, peak memory usage, and allocation churn.
