...
public:
  DummyProcess(CompilationContext &ctx)
      : CompilationProcess(ctx, "DummyLexer") {}
...
TEST_CASE("Diagnostic Process creates sub loggers successfully",
          "[new_architecture]") {
...
TEST_CASE("CompilationProcess uses diagnostic logger", "[new_architecture]") {
  CompilationOptions options;
  // We can't easily capture stdout without redirecting standard streams or
  // using custom sinks, but we can ensure it compiles and runs without
  // crashing.
