...
TEST_CASE("Phase 31 - High-Performance Engine Tuning", "[phase31]") {
    SECTION("Uninterrupted OpCode dispatch and inlined pointer stack") {
    ...
    SECTION("Fixed zero-allocation call stack recursion") {
        CompilationOptions options;
        options.log_level = CompilationOptions::LogLevel::OFF;
        Source src_key = std::string("test");
        options.sources[src_key] = R"(
            public class RecurseTest {
                public static int32 sum_recurse(int32 n) {
                    if (n <= 0) return 0;
                    return n + sum_recurse(n - 1);
                }

                public static int32 main() {
                    return sum_recurse(100);
                }
            }
        )";
