...
TEST_CASE("New Lexer - Primitives and Strings", "[new_lexer]") {
    auto tokens = test_tokenize("int32 x = 5; char[] s = \"hello\"; string y = \"world\";");
    // int32, x, =, 5, ;, char, [], s, =, "hello", ;, string, y, =, "world", ;, EOF
...
TEST_CASE("New Lexer - Classes, Functions and Enums", "[new_lexer]") {
    auto tokens = test_tokenize("public class MyClass { public void test() {} } enum State { A, B }");
...
TEST_CASE("New Lexer - Full test.slx script", "[new_lexer]") {
    auto tokens = test_tokenize_file("/home/jehud/Projects/solix/tests/resources/test.slx");
