...
1. **Unstructured Test Organization**:
   - All tests live in `tests/src/` with legacy chronological names (`test_phase20.cpp` through `test_phase34.cpp`). It is impossible to identify what subsystem a file tests without reading the source.
2. **No Data-Driven End-to-End Test Harness**:
   - Tests rely on embedding large Solix programs as raw multiline C++ strings and manually driving internal pipeline phases.
3. **Missing Standard Library Verification in `ctest`**:
   - Standard library test scripts in `scratch/` are not run by `ctest`.
4. **CMake Redundancy**:
   - `tests/CMakeLists.txt` combines `file(GLOB_RECURSE)` with explicit `target_sources()`.

### Solution
1. **Domain-Based Directory Restructuring**:
   - Reorganize `tests/`:
     ```
     tests/
     ├── unit/
     │   ├── lexer/          # Tokenization, literals, operators
     │   ├── parser/         # Grammar, AST nodes, error recovery
     │   ├── binder/         # Type checking, templates, vtables
     │   ├── assembler/      # Bytecode emission, labels, opcodes
     │   └── runtime/        # VM execution, ARC memory, exception unwinding
     ├── stdlib/             # Collections, core, math, systems tests
     ├── e2e/                # Data-driven runner executing .slx fixtures
     └── fixtures/           # Standalone .slx files with expected outputs
     ```
2. **Rename Legacy Test Files to Semantic Names**:
   - Map `test_phaseXX.cpp` to descriptive names:
     - `test_phase20.cpp` $\to$ `unit/binder/test_function_templates.cpp`
     - `test_phase21.cpp` $\to$ `unit/binder/test_template_deduction.cpp`
     - `test_phase22.cpp` $\to$ `unit/binder/test_type_resolution_and_null.cpp`
     - `test_phase23.cpp` $\to$ `unit/lexer/test_reference_operators.cpp`
     - `test_phase24.cpp` $\to$ `unit/assembler/test_typed_alu_opcodes.cpp`
     - `test_phase25.cpp` $\to$ `unit/runtime/test_memory_integrity_arrays.cpp`
     - `test_phase26.cpp` $\to$ `unit/runtime/test_arc_ownership.cpp`
     - `test_phase27.cpp` $\to$ `unit/runtime/test_cleanup_and_control_flow.cpp`
     - `test_phase28.cpp` $\to$ `unit/runtime/test_object_init_and_arrays.cpp`
     - `test_phase29.cpp` $\to$ `unit/runtime/test_string_pool.cpp`
     - `test_phase30.cpp` $\to$ `unit/binder/test_nested_templates.cpp`
     - `test_phase31.cpp` $\to$ `unit/runtime/test_engine_performance.cpp`
     - `test_phase32.cpp` $\to$ `unit/runtime/test_runtime_vm_memory.cpp`
     - `test_phase33.cpp` $\to$ `unit/assembler/test_assembler_codegen_arc.cpp`
     - `test_phase34.cpp` $\to$ `unit/binder/test_binder_types_vtables.cpp`
3. **Data-Driven E2E Test Harness**:
   - Create a test runner that scans `tests/fixtures/*.slx` and parses declarative directives:
     ```slx
     // RUN: solix compile %s -o %t.slxb && solix run %t.slxb
     // EXPECT_EXIT: 0
     // EXPECT_STDOUT: "Calculation passed: 42"
     ```
4. **Standard Library `ctest` Suite**:
   - Add Catch2 test cases compiling and running all collections, core utilities, and exceptions.
   - Clean up `tests/CMakeLists.txt` to eliminate duplicate source specifications.
